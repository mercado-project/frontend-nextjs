...
const SingleGridItem = ({ item }: { item }) => {
...
const handleQuickViewUpdate = () => {
  ...
  const promo = item.promotions?.find((p: any) => p.active);
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden flex items-center justify-center rounded-lg bg-white shadow-1 min-h-[270px] mb-4">
        <Link href={`/${item.url}`}>
          <Image className="w-full h-[250px]" src={mainImage} alt={item.name} width={250} height={250} />
        </Link>
